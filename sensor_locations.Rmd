...
#unique(first_day_list$sensor_type) %>%
#  purrr::walk(~dir.create(paste0("data/",.x)))

#first_day_list[1:10,] %>%
#    group_by(first_day,sensor_id) %>%
#    group_walk(~ {read_delim(.x$file_url,";",n_max=1) %>% #write_delim(paste0("data/",.x$sensor_type,".csv"),delim=";",append=TRUE,col_names=TRUE)})
